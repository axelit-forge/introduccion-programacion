...
short menuAB(conjunct, conjunct);
...
void inicializacion( conjunct*A, conjunct*B){
	...
	printf("\nIngrese el nombre del primer conjunto (recuerde que es un solo caracter): ");
	scanf("%c", &name);
	A->name= name;
	fflush(stdin);
	
	printf("\nIngrese el nombre del segundo conjunto (recuerde que es un solo caracter): ");
		scanf("%c", &name);
	B->name= name;
	fflush(stdin);
...
short menuAB(conjunct A, conjunct B){
	short b;
	char opcion;
	...
	}while (opcion!= A.name && opcion!= B.name);
